...
async fn validate_citation_with_ai(
    ...
    // First check if the URL exists and resolve any redirects
    match news_interjection::validate_url_exists(citation).await {
        Ok((true, Some(final_url))) => {
            // URL exists and we have the final URL after redirects
            // Validate the fact against the final URL, not the original
            match validate_fact_matches_citation(gemini_client, fact, &final_url).await {
                Ok(matches) => {
                    if matches {
        ...
        Ok((true, None)) => {
            // URL exists but no final URL returned - validate against original
...
async fn validate_citation_with_fallback(
    ...
    match news_interjection::validate_url_exists(citation).await {
        Ok((true, Some(final_url))) => {
            // URL exists and we have the final URL after redirects
            info!("Citation URL exists: {} -> {}", citation, final_url);

            // Validate that the fact matches the final URL content, not the original
            match validate_fact_matches_citation(gemini_client, fact, &final_url).await {
                Ok(true) => {
                    info!("Fact matches citation content: {}", final_url);
                    Ok((true, Some(final_url)))
                }
                Ok(false) => {
                    info!("Fact does NOT match citation content: {}. Attempting to find a better URL...", final_url);
        ...
        Ok((true, None)) => {
            // URL exists but no final URL returned - validate against original
            info!("Citation URL exists but no redirect info: {}", citation);
            match validate_fact_matches_citation(gemini_client, fact, citation).await {
                Ok(true) => {
