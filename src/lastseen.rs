    ...
    if let Some(conn) = db_conn {
        ...
        match finder.find_last_message(conn.clone(), name).await {
            Ok(Some((author, display_name, content, timestamp))) => {
                ...
                let response = format!("{} was last seen {} ago, saying: \"{}\"", 
            ...
            Ok(None) => {
                if let Err(e) = msg.channel_id.say(http, format!("I haven't seen anyone matching \"{}\"", name)).await {
                    error!("Error sending no match message: {:?}", e);
                }
            },
            Err(e) => {
                error!("Error finding last message: {:?}", e);
                if let Err(e) = msg.channel_id.say(http, "Error searching message history").await {
                    error!("Error sending error message: {:?}", e);
                }
            }
        }
    } else {
        if let Err(e) = msg.channel_id.say(http, "Message history database is not available").await {
