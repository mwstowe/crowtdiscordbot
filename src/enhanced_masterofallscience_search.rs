...
const GEMINI_MASTEROFALLSCIENCE_PROMPT: &str = r#"You are a Rick and Morty quote expert tasked with finding the most relevant quote based on search terms.
...
Examples:
- Search: "wubba lubba"
- Response: {{"quote": "Wubba lubba dub dub!", "episode": "Season 1 Episode 11: Ricksy Business", "character": "Rick Sanchez"}}

- Search: "pickle"
- Response: {{"quote": "I'm Pickle Rick!", "episode": "Season 3 Episode 3: Pickle Rick", "character": "Rick Sanchez"}}
...
impl EnhancedMasterOfAllScienceSearch {
    ...
    pub async fn search(&self, query: &str) -> Result<Option<MasterOfAllScienceResult>> {
        info!("Enhanced Master of All Science search for: {}", query);
        
        // Try direct site search on masterofallscience.com
        let direct_site_search = format!("site:masterofallscience.com {}", query);
        ...
        match self.google_client.search(&direct_site_search).await {
            Ok(Some(result)) => {
                ...
                // Try each search term with Master of All Science
        ...
        let gemini_prompt = GEMINI_MASTEROFALLSCIENCE_PROMPT.replace("{}", query);
                        ...
                        Ok(json) => {
                            ...
                            if !quote.is_empty() {
                                ...
                                if let Some(season_episode) = extract_season_episode_from_text(episode) {
                                    ...
                                    match self.masterofallscience_client.search(&season_episode).await {
                                        Ok(Some(result)) => {
        ...
        match self.masterofallscience_client.search(query).await {
            Ok(Some(result)) => {
                if self.result_contains_search_terms(&result, query) {
                    Ok(Some(result))
                } else {
    ...
    fn calculate_total_score(&self, result: &MasterOfAllScienceResult, query: &str, search_term: &str) -> f32 {
    ...
    fn result_contains_search_terms(&self, result: &MasterOfAllScienceResult, query: &str) -> bool {
    ...
    fn calculate_popularity_score(&self, _result: &MasterOfAllScienceResult) -> f32 {
