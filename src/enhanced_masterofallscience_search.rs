...
use crate::masterofallscience::{MasterOfAllScienceClient, MasterOfAllScienceResult};
...
#[derive(Debug)]
struct RankedMasterOfAllScienceResult {
    result: MasterOfAllScienceResult,
    relevance_score: f32,
...
const GEMINI_MASTEROFALLSCIENCE_PROMPT: &str = r#"
You are helping to search for Rick and Morty quotes and scenes. Given a user's search query, generate 3-5 possible exact phrases or quotes from Rick and Morty that best match what the user is looking for.
...
1. Include character names if they're relevant (e.g., "Rick", "Morty", "Summer", "Beth", "Jerry")
...
Examples:
- Query: "wubba lubba" → "Wubba lubba dub dub!"
- Query: "pickle rick" → "I'm Pickle Rick!"
- Query: "get schwifty" → "Get schwifty!"
- Query: "tiny rick" → "I'm Tiny Rick!"
- Query: "peace among worlds" → "Peace among worlds"
- Query: "plumbus" → "Everyone has a plumbus in their home"
- Query: "meeseeks" → "I'm Mr. Meeseeks, look at me!"
...
pub struct EnhancedMasterOfAllScienceSearch {
    gemini_client: GeminiClient,
    masterofallscience_client: MasterOfAllScienceClient,
    google_client: GoogleSearchClient,
}

impl EnhancedMasterOfAllScienceSearch {
    pub fn new(gemini_client: GeminiClient, masterofallscience_client: MasterOfAllScienceClient, google_client: GoogleSearchClient) -> Self {
        Self {
            gemini_client,
    ...
    pub async fn search(&self, query: &str) -> Result<Option<MasterOfAllScienceResult>> {
        info!("Enhanced MasterOfAllScience search for: {}", query);
    ...
    async fn find_quotes_via_search(&self, query: &str) -> Result<Vec<String>> {
        ...
        for search_query in &search_queries {
            info!("Searching for Rick and Morty quotes with query: {}", search_query);
    ...
    fn extract_potential_quotes(&self, title: &str, snippet: &str) -> Vec<String> {
        ...
        // Check if the text contains Rick and Morty-related keywords
        if combined_text.to_lowercase().contains("rick") || combined_text.to_lowercase().contains("morty") {
            ...
            for sentence in potential_sentences {
                ...
                let character_names = ["rick", "morty", "summer", "beth", "jerry", "unity", "birdperson", "squanchy"];
            ...
            if let Ok(re) = Regex::new(pattern) {
                for caps in re.captures_iter(text) {
                    if let Some(quote) = caps.get(1) {
                        ...
                        // Skip quotes that are likely not from Rick and Morty
    ...
    async fn generate_search_terms(&self, query: &str) -> Result<Vec<String>> {
        let prompt = GEMINI_MASTEROFALLSCIENCE_PROMPT.replace("{query}", query);
    ...
    fn calculate_relevance_score(&self, result: &MasterOfAllScienceResult, query: &str) -> f32 {
    ...
    fn calculate_popularity_score(&self, result: &MasterOfAllScienceResult) -> f32 {
        ...
        let iconic_characters = [
            ...
            "meeseeks", "pickle", "tiny rick", "evil morty", "mr. poopybutthole"
        ];
        ...
        let iconic_phrases = [
            "wubba lubba", "pickle rick", "get schwifty", "tiny rick", "peace among worlds",
            "plumbus", "meeseeks", "show me what you got", "szechuan sauce", "portal gun",
            "interdimensional", "morty's mind blowers", "rick and morty"
        ];
    ...
    fn calculate_quote_match_score(&self, result: &MasterOfAllScienceResult, search_term: &str) -> f32 {
