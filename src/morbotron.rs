...
impl MorbotronClient {
    ...
    async fn search_with_strategy(&self, query: &str) -> Result<Option<MorbotronResult>> {
        ...
        let mut best_result: Option<MorbotronResult> = None;
        ...
        for (i, result) in search_results.iter().take(5).enumerate() {
            ...
            match self.get_caption_for_frame(episode, timestamp).await {
                Ok(Some(morbotron_result)) => {
                    ...
                    if score > best_score {
                        best_score = score;
                        best_result = Some(morbotron_result);
                    }
                },
                _ => continue
            }
