...
const RANDOM_SEARCH_TERMS: &[&str] = &[
    ...
    "tiny rick",
    "old rick",
    "gotta save tiny rick",
    "help me morty",
    "help me summer",
    "i'm gonna die",
    "tiny rick is dying",
    "let me out of here",
    "i'm old rick trapped in a young body",
    "tiny rick",
    "fuck tiny rick",
    "i love tiny rick",
    "save me",
    "let me out",
    "help me",
    "i'm trapped in a vat",
    "i'm dying",
    "i'm going to die",
    "i'm going to die in here",
    "i'm going to die in a vat",
    "i'm going to die in the garage",
    "i'm going to die in a vat in the garage",
];
...
impl MasterOfAllScienceClient {
    ...
    async fn get_caption_for_frame(
        ...
        let caption_result: MasterOfAllScienceCaptionResult = caption_response
            .json()
            .await
    ...
    async fn search_api(&self, query: &str) -> Result<Vec<MasterOfAllScienceSearchResult>> {
        ...
        if !status.is_success() {
            return Err(anyhow!(
                "MasterOfAllScience search failed with status: {}",
                status
            ));
        }
        ...
        let search_results: Vec<MasterOfAllScienceSearchResult> =
            match serde_json::from_str::<Vec<MasterOfAllScienceSearchResult>>(&response_body) {
                Ok(results) => {
                    info!(
                        "Successfully parsed MasterOfAllScience search results: {} results",
                        results.len()
                    );
                    results
                }
                Err(e) => {
                    error!(
                        "Failed to parse MasterOfAllScience search results: {}. Response body: {}",
                        e, response_body
                    );
                    return Err(anyhow!(
                        "Failed to parse MasterOfAllScience search results: {}",
                        e
                    ));
                }
            };
...
pub async fn handle_masterofallscience_command(
    ...
    if args.is_none() {
        ...
        let searching_msg = match msg
            .channel_id
            .say(http, "Finding a random Rick and Morty moment...")
            .await
        {
            Ok(msg) => Some(msg),
            Err(e) => {
        ...
        match masterofallscience_client.random().await {
            ...
            Ok(None) => {
                let error_msg =
                    "Couldn't find any Rick and Morty screenshots. Wubba lubba dub dub!";
    ...
    if let Some(term) = args {
        ...
        let searching_msg = match msg
            .channel_id
            .say(http, "🔍 Searching Rick and Morty quotes...")
            .await
        {
            Ok(msg) => Some(msg),
            Err(e) => {
